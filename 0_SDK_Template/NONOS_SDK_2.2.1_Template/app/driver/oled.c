...
OLED_DrawPicture(u8 x0, u8 y0, u8 x1, u8 y1, u8*Picture)
{
	u8 T_x, T_y; 					// 定义变量

	u32 N_Picture_Element = 0;		// 图片数组的字节元素

	for(T_y=y0; T_y<=y1; T_y++)		// 从起始页开始，画到终止页
	{
		OLED_Set_Pos(x0,T_y); 		// 在页的起始列开始画

		for(T_x=x0;T_x<=x1;T_x++) 	// 画x1 - x0 列
		{
			OLED_WR_Byte(Picture[N_Picture_Element++],OLED_DATA);	// 画点
		}
	}
}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void ICACHE_FLASH_ATTR
OLED_DrawPicture_WIFI(u8 x0, u8 y0, u8 x1, u8 y1)
{
	u8 T_x, T_y; 					// 定义变量
	...
	for(T_y=y0; T_y<=y1; T_y++)		// 从起始页开始，画到终止页
	{
		...
		for(T_x=x0;T_x<=x1;T_x++) 	// 画x1 - x0 列
		{
			OLED_WR_Byte(Picture_WIFI[N_Picture_Element++],OLED_DATA);	// 画点
		}
	}
}
//-----------------------------------------------------------------------------
